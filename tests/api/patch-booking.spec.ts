  ...
  () => {
    test(
      ...
      async ({ bookingApi, authApi }) => {
        ...
        const patchResponse = await bookingApi.partialUpdateBooking(
          bookingId,
          {
            firstname: 'Onur',
          },
        ...
        const patchedBody = (await patchResponse.json()) as Booking;

        expect(patchedBody.firstname).toBe('Onur');

        // Değişmeyen alanlar korunmalı
        expect(patchedBody.lastname).toBe(booking.lastname);
      },
    );
  },
);
