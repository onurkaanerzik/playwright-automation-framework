...
import type { Booking, CreateBookingResponse } from '../../src/api';
  ...
  () => {
    test(
      ...
      async ({ bookingApi }) => {
        ...
        const bookingResponse = (await response.json()) as Booking;

        expect(bookingResponse.firstname).toBeTruthy();

        expect(bookingResponse.lastname).toBeTruthy();

        expect(typeof bookingResponse.totalprice).toBe('number');

        expect(typeof bookingResponse.depositpaid).toBe('boolean');

        expect(bookingResponse.bookingdates.checkin).toBeTruthy();

        expect(bookingResponse.bookingdates.checkout).toBeTruthy();
      },
    );
  },
);
